...
async function validateSubControl(subControl: (typeof subControlsToImport)[0]) {
  ...
  if (!subControlIdPattern.test(subControl.subControlId)) {
    throw new Error(
      `Invalid sub-control ID format for ${subControl.subControlId}`
    );
  }
  ...
  if (!controlIdPattern.test(subControl.controlId)) {
    throw new Error(
      `Invalid parent control ID format for ${subControl.controlId}`
    );
  }
}
...
async function importSubControls() {
  try {
    ...
    const results = await Promise.all(
      subControlsToImport.map(async (subControl) => {
        try {
          const filter = { subControlId: subControl.subControlId };
          const update = { ...subControl };
          const options = { upsert: true, new: true };

          const doc = await SubControl.findOneAndUpdate(
            filter,
            update,
            options
          );
    ...
    results.forEach((result) => {
      if (result.status === 'success') {
        const action = 'action' in result ? result.action : 'processed';
        console.log(`✅ ${result.subControlId} ${action} successfully`);
      } else {
...
export default importSubControls;
